function plotVhAsAFunctionOfTime_C(scenarioPath, cityName, insertionRate, xL, yL, fileName)
    X = [];
    Y = [];
    for vhKind = {'HD', 'AV', 'CAV'}
        X = [ X, insertionRate' ];
        for ll_ = 1:length(insertionRate)
            load( [ scenarioPath '/' cityName '-' cell2mat(vhKind) '-uncapped-ir-' num2str(insertionRate(ll_)) '.mat'], 'vehiclesAll' );
    ...
    utils.plotsparsemarkers(PL,LG,{MRK{1}, MRK{2}, MRK{3}},50, true, [10,10,10],true);
